...
//@SeeAlso({ConsumePulsarBose.class, ConsumePulsar.class, ConsumePulsarRecord.class, PublishPulsarRecord.class})
@SeeAlso({ConsumePulsarBose.class})
...
@TriggerWhenEmpty
@ReadsAttributes({
    @ReadsAttribute(attribute = "message.header.<headerName>", description = "Message property for the Pulsar message(s)"),
    @ReadsAttribute(attribute = "message.eventTime", description = "The event time"),
    @ReadsAttribute(attribute = "message.key", description = "The partition key value to use"),
    @ReadsAttribute(attribute = "message.sequenceId", description = "Sequence ID to specify for the Pulsar message(s)")
})

public class PublishPulsarBose extends AbstractPulsarProducerProcessor<byte[]> {

    @Override
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {

        // TODO: Async does not work due to protected member access
    ...
    private void send(Producer<byte[]> producer, ProcessSession session, FlowFile flowFile, byte[] demarcatorBytes) throws PulsarClientException {
        ...
        try (final InputStream in = session.read(flowFile); final StreamDemarcator demarcator = new StreamDemarcator(in, demarcatorBytes, Integer.MAX_VALUE)) {
           while ((messageContent = demarcator.nextToken()) != null) {
                TypedMessageBuilder<byte[]> mb = producer.newMessage().value(messageContent);
                Map<String,String> attributes = flowFile.getAttributes();
                if (attributes != null) {
                    for (String k: attributes.keySet()) {
                        switch (k) {
                            case "message.key":
                                mb.key(attributes.get(k));
                                break;
                            case "message.eventTime":
                                Long ts = Long.getLong(attributes.get(k));
                                if (ts != null) {
                                    mb.eventTime(ts);
                                }
                                break;
                            case "message.sequenceId":
                                Long sid = Long.getLong(attributes.get(k));
                                if (sid != null) {
                                    mb.sequenceId(sid);
                                }
                                break;
                            default:
                                if (k.startsWith("message.header.")) {
                                    String headerName = k.substring(15);
                                    mb.property(headerName, attributes.get(k));
                                }
                        }
                    }
                }
                // TODO: Do we want to save the messageId?
                if (mb.send() != null) {
